...
public class InputStreamRequest extends Request<byte[]> {
     ...
     */
    public InputStreamRequest(int method, String url, Response.Listener<byte[]> listener, ErrorListener errorListener) {
     ...
     * @param method the request {@link Method} to use
     * @param url URL to fetch the string at
     * @param data byte[] to upload
     * @param listener Listener to receive the String response
     * @param errorListener Error listener, or null to ignore errors
     */
    public InputStreamRequest(int method, String url, byte[] data, Response.Listener<byte[]> listener, ErrorListener errorListener) {
        super(method, url, errorListener);
        // this request would never use cache.
        setShouldCache(false);
        mListener = listener;
