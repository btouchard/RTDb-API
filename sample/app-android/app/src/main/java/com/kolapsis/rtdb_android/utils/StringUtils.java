package com.kolapsis.rtdb_android.utils;
...
public class StringUtils {
	...
	public static String size(double size, boolean si){
		...
		String str = " octets";
		//"+(si?"i":"")+"
		if (size > unit){size /= unit; str = " Ko";}
		if (size > unit){size /= unit; str = " Mo";}
		if (size > unit){size /= unit; str = " Go";}
		if (size > unit){size /= unit; str = " To";}
		NumberFormat formater = new DecimalFormat("#.##");
...
     * */
	...
	public static String getDate(long date, boolean small) {
		SimpleDateFormat sdf = new SimpleDateFormat(small?"dd/MM/yyyy":"dd MMMM yyyy", Locale.FRANCE);
		return sdf.format(new Date(date));
	}
	...
	public static long getDateTimeFromMySQL(String str) {
		long date = 0;
	...
	//public static String getDateToMySQL(long date) {
	...
	public static String removeExtension(String name) {
		...
		if (name.indexOf(".") > -1) name.substring(0, name.lastIndexOf("."));
		return name;
	}
